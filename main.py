...
parser.add_argument("--use_sim3", action="store_true", help="Use Sim3 (7DoF: rotation + translation + scale) instead of SE3 for loop closure and submap alignment. Recommended for monocular long-range sequences.")
...
def main():
    ...
    solver = Solver(
        ...
        use_sim3=args.use_sim3          # Sim3(7DoF) vs SE3(6DoF) for LC & submap alignment
    )
    print(f"Loop closure transform: {'Sim3 (7DoF, scale-aware)' if args.use_sim3 else 'SE3 (6DoF, fixed-scale)'}")
