...
class Solver:
    def __init__(self,
        ...
        use_sim3: bool = False):      # Sim3(7DoF) vs SE3(6DoF) for submap alignment
        
        self.init_conf_threshold = init_conf_threshold
        self.vis_voxel_size = vis_voxel_size
        self.use_sim3 = use_sim3      # If True, propagate scale into inner-submap nodes
    ...
    def add_edge(self, submap_id_curr, frame_id_curr, submap_id_prev=None, frame_id_prev=None, is_loop_closure=False):
        ...
        for index, pose in enumerate(world_to_cam):
            ...
            # ── Sim3: propagate inter-submap scale into inner-submap translation ──
            # SE3 (default): H_inner is pure rigid (no scale).
            # Sim3: scale the translation part of H_inner so that all nodes in this
            # submap are expressed in the same metric scale as the previous submap.
            if self.use_sim3 and scale_factor != 1.0:
                H_inner_sim3 = H_inner.copy()
                H_inner_sim3[:3, 3] *= scale_factor   # scale translation component only
                current_node = self.graph.get_homography(submap_id_curr + index - 1) @ H_inner_sim3
                self.graph.add_homography(submap_id_curr + index, current_node)
                self.graph.add_between_factor(submap_id_curr + index - 1, submap_id_curr + index, H_inner_sim3, self.graph.inner_submap_noise)
            else:
    ...
    def run_predictions(self, image_names, model, max_loops, clip_model, clip_preprocess, use_sim3=False):
